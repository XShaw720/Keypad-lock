 ...
 *    press the unlock button while its unlock to set passcode,
 *        after input new passcode press lock to update
 */
...
void loop() {
  int entries[8], passcode[8], i, butPressed;
  ...
  while(1){ //unlocked loop
    ...
    if(butPressed==lock){
      //action to lock
      flash(red, 3, 200); //flash light to show it's locked
